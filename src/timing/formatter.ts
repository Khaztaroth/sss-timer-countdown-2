...
export function useFormatter(timeDiff: Duration) {
    if (timeDiff.days > 0) {
        return (
            timeDiff.toFormat(
                `d '${timeDiff.days === 1 ? 'day' : 'days'}' h '${timeDiff.hours === 1 ? 'hour' : 'hours'}' m '${timeDiff.minutes === 0 ? "" : timeDiff.minutes === 1 ? 'minute' : 'minutes'}'`
            )
        )
    } else if (timeDiff.hours >= 0) {
        return (
            timeDiff.toFormat(
                `h '${timeDiff.hours === 1 ? 'hour' : 'hours'}' m '${timeDiff.minutes === 0 ? "" : timeDiff.minutes === 1 ? 'minute' : 'minutes'}'`
            )
        )
